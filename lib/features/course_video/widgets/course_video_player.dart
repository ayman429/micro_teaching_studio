...
class CourseVideoHandle {
  CourseVideoCubit? _cubit;
...
class CourseVideoPlayer extends StatelessWidget {
  ...
  final CourseVideoHandle? handle;

  @override
  Widget build(BuildContext context) {
    return BlocProvider(
      create: (_) {
        final cubit = CourseVideoCubit()..load(asset);
        handle?._cubit = cubit;
        return cubit;
      },
...
class _CourseVideoCardState extends State<_CourseVideoCard>
  ...
  Widget build(BuildContext context) {
    return BlocListener<CourseVideoCubit, CourseVideoState>(
      ...
      child: BlocListener<CourseVideoCubit, CourseVideoState>(
        listenWhen: (previous, current) =>
            !previous.completed && current.completed,
        listener: (context, state) => widget.onCompleted?.call(),
        child: BlocListener<CourseVideoCubit, CourseVideoState>(
          listenWhen: (previous, current) =>
              !previous.isPlaying && current.isPlaying,
          listener: (context, state) => widget.onPlaybackStarted?.call(),
          child: Container(
