...
class ClassroomQuizCard extends StatelessWidget {
  const ClassroomQuizCard({super.key});

  @override
  Widget build(BuildContext context) {
    return Directionality(
      textDirection: ui.TextDirection.ltr,
      child: BlocBuilder<ClassroomQuizCubit, ClassroomQuizState>(
              ...
              Container(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(
                      AppStrings.classroomQuizPrompt.tr(),
                      style: getBoldStyle(
                    ...
                    SizedBox(height: AppPadding.p16.h),
                    for (var index = 0;
                        index < ClassroomScript.quizItems.length;
                        index++) ...[
                      _QuestionBlock(
                        index: index,
                        ...
                        onSelected: (choice) => context
                            .read<ClassroomQuizCubit>()
                            .select(index, choice),
                      ),
                      if (index < ClassroomScript.quizItems.length - 1)
                        SizedBox(height: AppPadding.p16.h),
                    ],
                    ...
                    SizedBox(
                      ...
                      child: DefaultButtonWidget(
                        onPressed: state.canSubmit
                            ? () => context.read<ClassroomQuizCubit>().submit()
                            : null,
...
class _QuestionBlock extends StatelessWidget {
  ...
  @override
  Widget build(BuildContext context) {
    final item = ClassroomScript.quizItems[index];
    return Column(
      crossAxisAlignment: CrossAxisAlignment.start,
      children: [
        Text(
        ...
        for (var choice = 0; choice < item.choices.length; choice++) ...[
          ...
          if (choice < item.choices.length - 1) SizedBox(height: AppPadding.p8.h),
        ],
      ],
    );
  }
...
class _FeedbackBanner extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final key = switch (tone) {
      QuizTone.correct => AppStrings.classroomQuizCorrect,
      QuizTone.review => AppStrings.classroomQuizRetry,
      _ => AppStrings.classroomQuizWrong,
    };
    return Container(
      width: double.infinity,
